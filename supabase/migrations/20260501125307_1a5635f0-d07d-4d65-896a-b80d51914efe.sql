CREATE OR REPLACE FUNCTION public.calculate_football_points(predicted_home integer, predicted_away integer, actual_home integer, actual_away integer, scoring_system text DEFAULT 'standard'::text)
 RETURNS integer
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF predicted_home = actual_home AND predicted_away = actual_away THEN
    ...
    ELSE
      RETURN 3;  -- standard system: 3 points for exact score
    END IF;
  END IF;
  ...
  -- Simplified and Standard system: only correct result = 1 point
