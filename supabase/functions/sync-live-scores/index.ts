...
// Bidirectional EN↔PT team name mapping for reconciliation
const TEAM_NAME_ALIASES: Record<string, string[]> = {
  ...
  'central african republic': ['republica centro-africana', 'republica centro africana'],
  'chad': ['chade'],
  'china': ['china'],
  'china pr': ['china'],
  'colombia': ['colombia'],
  ...
  'grenada': ['granada'],
  'guinea': ['guine'],
  'guinea-bissau': ['guine-bissau', 'guine bissau'],
  'guyana': ['guiana'],
  'honduras': ['honduras'],
  'hong kong': ['hong kong'],
  'hungary': ['hungria'],
  ...
  'zimbabwe': ['zimbabue'],
  // Brazilian clubs
  'ca paranaense': ['athletico-pr', 'athletico pr', 'athletico paranaense'],
  'atletico paranaense': ['athletico-pr', 'athletico pr'],
  ...
  'gremio fbpa': ['gremio'],
  'gremio rs': ['gremio'],
  'clube atletico mineiro': ['atletico-mg', 'atletico mg', 'atletico mineiro'],
  'atletico mineiro': ['atletico-mg', 'atletico mg'],
  'cruzeiro ec': ['cruzeiro'],
  'ec bahia': ['bahia'],
  'ec vitoria': ['vitoria'],
  'sport club do recife': ['sport', 'sport recife'],
...
// Build reverse lookup: normalized alias → list of normalized canonical names
const ALIAS_REVERSE_MAP: Map<string, string[]> = new Map();
for (const [canonical, aliases] of Object.entries(TEAM_NAME_ALIASES)) {
  const normCanonical = canonical.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase();
  for (const alias of aliases) {
    const normAlias = alias.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase();
    // Map alias → canonical
    if (!ALIAS_REVERSE_MAP.has(normAlias)) ALIAS_REVERSE_MAP.set(normAlias, []);
    ALIAS_REVERSE_MAP.get(normAlias)!.push(normCanonical);
    // Map canonical → alias (bidirectional)
    if (!ALIAS_REVERSE_MAP.has(normCanonical)) ALIAS_REVERSE_MAP.set(normCanonical, []);
    ALIAS_REVERSE_MAP.get(normCanonical)!.push(normAlias);
  }
}
...
function scoreTeamSimilarity(apiName: string, targetName: string): number {
  ...
  if (apiAliases.length > 0 && targetAliases.length > 0) {
    const apiAliasSet = new Set(apiAliases);
    if (targetAliases.some(a => apiAliasSet.has(a))) return 0.99;
  }
