...
const AdminUserManagement = () => {
  ...
  const getRoleBadge = (role: string) => {
    switch (role) {
      ...
      case "in_app_payment": return <Badge className="text-xs bg-emerald-600 text-white">Pagamento por comprovante</Badge>;
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  >
                    {actionLoading === `role-${user.id}-in_app_payment` ? (
                      <Loader2 className="w-3 h-3 animate-spin mr-1" />
                    ) : user.roles.includes("in_app_payment") ? (
                      <Wallet className="w-3 h-3 mr-1" />
                    ) : (
                      <Wallet className="w-3 h-3 mr-1" />
                    )}
                    {user.roles.includes("in_app_payment") ? "Remover Pagamento por Comprovante" : "Ativar Pagamento por Comprovante"}
                  </Button>
